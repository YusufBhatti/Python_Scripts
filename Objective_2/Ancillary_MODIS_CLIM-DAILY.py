...
mean_chl=np.empty((first_dim,sec_dim,lat_num,lon_num))
count=0
modis_bins=[]
mean_chl=np.empty((first_dim,sec_dim,lat_num,lon_num)); mean_chl[:]=np.nan
std_chl=np.empty((first_dim,sec_dim,lat_num,lon_num)); std_chl[:]=np.nan
median_chl=np.empty((first_dim,sec_dim,lat_num,lon_num)); median_chl[:]=np.nan
IQR=np.empty((first_dim,sec_dim,lat_num,lon_num)); IQR[:]=np.nan
...
for m in range(0,first_dim):
    for d in range(0,sec_dim):
        ...
        for la in (laty):
            ...
            for lo in (lony):
                lo_co=lo_co+1
                data=(modis_new_data[m,d,la:la+interval,lo:lo+interval])
                latitude[la_co]=np.nanmean(modis_new_lat[la:la+interval])
                longitude[lo_co]=np.nanmean(modis_new_lon[lo:lo+interval])
                median_chl[m,d,la_co,lo_co]=np.nanmedian(data)
                std_chl[m,d,la_co,lo_co]=np.nanstd(data)
                mean_chl[m,d,la_co,lo_co]=np.nanmean(data)
                Q1=np.nanquantile(data,.25); Q3=np.nanquantile(data,.75)
                IQR[m,d,la_co,lo_co]=Q3-Q1
...
Data_Type='Daily_2020_1080x2158'
