...
olat=modis_data.lat.data
olon=modis_data.lon.data              

# model_mean_chl=np.nanmean(medusa_chl,axis=0)*1e6
# lat=medusa_chl.latitude.data
# lon=medusa_chl.longitude.data

### CALCULATES CDF FOR EACH 1 X 1 GRID CELL ########################
### CALCULATES CDF FOR EACH 1 X 1 GRID CELL ########################
#chl=np.nanmedian(month_chl,axis=0)
#modis_check=check_lev_lat_lon_model(olat,olon,month_chl) # for DAILY DATA
modis_check=check_lat_lon_model(olat,olon,month_chl) # FOr MONTHLY data
modis_new_lat=modis_check[0]
modis_new_lon=modis_check[1]
modis_new_data=modis_check[2]


latitude=np.empty((180)); latitude[:]=np.nan
longitude=np.empty((360)); longitude[:]=np.nan
mean_chl=np.empty((180,360))
count=0
modis_bins=[]
mean_chl=np.empty((180,360)); mean_chl[:]=np.nan
std_chl=np.empty((180,360)); std_chl[:]=np.nan
median_chl=np.empty((180,360)); median_chl[:]=np.nan
IQR=np.empty((180,360)); IQR[:]=np.nan
laty=np.arange(0,4320,24)
lony=np.arange(0,8629,24)
la_co=-1
lo_co=-1
for la in (laty):
    la_co=la_co+1
    for lo in (lony):
        lo_co=lo_co+1
        data=(modis_new_data[:,la:la+24,lo:lo+24])
        latitude[la_co]=np.nanmean(modis_new_lat[la:la+24])
        longitude[lo_co]=np.nanmean(modis_new_lon[lo:lo+24])
...
File_Path='/home/ybh10/Objective_2/Chlorophyll_Climatology/CLIMATOLOGY'
Data_Type='Chlorophyll_Climatology'

np.save('{}/Median_{}_ORCA1.npy'.format(File_Path,Data_Type),median_chl)   
np.save('{}/Mean_{}_ORCA1.npy'.format(File_Path,Data_Type),mean_chl) 
np.save('{}/STD_{}_ORCA1.npy'.format(File_Path,Data_Type),std_chl) 
np.save('{}/IQR_{}_ORCA1.npy'.format(File_Path,Data_Type),IQR)
#p.save('{}/{}_MODIS_GRID.npy'.format(File_Path,Data_Type),month_chl)
